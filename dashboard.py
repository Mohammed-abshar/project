...
with tab1:
    ...
    fig_trend = px.area(
        monthly_trend, x='Date', y='Monthly Cost',
    ...
    fig_vendor = px.pie(
        ...
        hole=0.5,
        color_discrete_sequence=['#8A2BE2', '#00F0FF', '#FF0055']
    )
    ...
    service_cost = filtered_df.groupby('Service Type')['Monthly Cost'].sum().reset_index().sort_values('Monthly Cost', ascending=True)
    fig_service = px.bar(
        service_cost, x='Monthly Cost', y='Service Type', orientation='h',
        title="Cost Breakdown by Service Type",
        color='Monthly Cost',
        color_continuous_scale=px.colors.sequential.Agsunset
    )
    fig_service.update_layout(**chart_layout, coloraxis_showscale=False)
    fig_service.update_xaxes(showgrid=True, gridcolor='#2D3748')
    fig_service.update_yaxes(showgrid=False)
